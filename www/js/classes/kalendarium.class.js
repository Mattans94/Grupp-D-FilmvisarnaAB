class Kalendarium extends Base {
	...
	renderShowTime(movieTitle, theater, id){
		...
		for (let i = 0; i < 3; i++){
			myLis += `<a href="${allShows[i].date} ${allShows[i].time}}<li><div class="col-6 pr-0 d-inline">${this.makeSmallDate(allShows[i].date)}</div> <div class="col-6 d-inline pl-1 float-right">${allShows[i].time}</div></li>`
		}
